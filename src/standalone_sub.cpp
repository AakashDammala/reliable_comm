/**
 * standalone_sub.cpp - Standalone Reliable Subscriber Node
 * A thin wrapper that hosts ReliableSub<T> as a ROS2 executable.
 * Supported message types: cdcl_umd_msgs/Observation, sensor_msgs/Image,
...
#include "reliable_comm/reliable_sub_v2.hpp"
...
int main(int argc, char* argv[])
{
  ...
  auto param_node = std::make_shared<rclcpp::Node>("standalone_sub_params");

  // Declare parameters with defaults
  param_node->declare_parameter<std::string>("topic_name", "/image");
  ...
  RCLCPP_INFO(param_node->get_logger(),
              "Starting standalone_sub: topic=%s, type=%s, qos=%zu",
              topic_name.c_str(),
              message_type.c_str(),
              qos_history_depth);
  ...
  std::string node_name = "reliable_sub_" + sanitized_topic;
  ...
  {
    auto reliable_sub_node =
        std::make_shared<ReliableSub<sensor_msgs::msg::Image>>(
            node_name,
            topic_name,
            qos_history_depth,
  ...
  {
    auto reliable_sub_node =
        std::make_shared<ReliableSub<geometry_msgs::msg::TwistStamped>>(
            node_name,
            topic_name,
            qos_history_depth,
            max_retries_per_loop,
  ...
  {
    auto reliable_sub_node =
        std::make_shared<ReliableSub<cdcl_umd_msgs::msg::Observation>>(
            node_name,
            topic_name,
            qos_history_depth,
            max_retries_per_loop,
